...
# 요청과 응답사이에 특정 작업 수행
# 미들웨이는 모든 요청에 대해 실행되며, 요청을 처리하기 전에 응답을 반환하기 전에 특정 작업을 수행할 수 있음
# 예를 들어 로깅, 인증, cars처리, 압축용...
import logging # 로깅 처리용 메서드
app = FastAPI(  # 앱의 시그니처와 환경설정을 담당
    title="MBC AI Study", # 앱의 제목
    description="MBC AI Study", # 앱의 주석(서렴ㅇ)
    version="0.0.1",    # 앱의 버전
...
class Item(BaseModel):  # 아이템 객체 생성(BaseModel : 객체 연결 -> 상속)
    ...
    price: float                # 가격 : 실수형
    tax: float                  # 세금 : 실수형(null)

@app.post("/items/") # post메서드용 요청 (create)
async def create_item(item: Item):
    # BaseModel은 데이터 모델링을 쉽게 도와주고 유효성검사도 수행
    # 잘못된 데이터가 들어오면 422 오류코드를 반환
    return item

@app.get("/")    # 웹 브라우저에 http://localhost:8001/ -> get요청시 처리
async def root():
    return {"Hello": "World"}



@app.get("/items/{item_id}") #http://localhost:8001/items/1 -> post요청
...
# 서버 실행은 uvicorn main:app --reload --part 8001
